    ...
    fun (commandQueue: MailboxProcessor<Msg>) (filter: ClArray<float32>) filterDiameter (image: ClArray<byte>) imageHeight imageWidth (result: ClArray<byte>) ->
        ...
        commandQueue.Post(
            Msg.MsgSetArguments(fun () ->
                kernel.KernelFunc ndRange image imageWidth imageHeight filter filterDiameter result
            )
        )
...
let rotateKernel (clContext: ClContext) localWorkSize =
    ...
    fun (commandQueue: MailboxProcessor<Msg>) (weight: ClCell<int>) (image: ClArray<byte>) imageHeight imageWidth (result: ClArray<byte>) ->
        ...
        commandQueue.Post(
            Msg.MsgSetArguments(fun () ->
                kernel.KernelFunc ndRange image imageWidth imageHeight weight result
            )
        )
...
let flipKernel (clContext: ClContext) localWorkSize =
...
let applyFilter (clContext: ClContext) (localWorkSize: int) =
    ...
    let applyFilterKernel = applyFilterKernel clContext localWorkSize
    let queue = clContext.QueueProvider.CreateQueue()

    fun (filter: float32[,]) (image: MyImage.MyImage) ->

        let input =
            clContext.CreateClArray<byte>(
                image.Data,
                HostAccessMode.NotAccessible,
                DeviceAccessMode.ReadOnly
            )
        ...
        let output =
            clContext.CreateClArray(
                ...
                * image.Width,
                HostAccessMode.NotAccessible,
                DeviceAccessMode.WriteOnly,
                allocationMode = AllocationMode.Default
            )

        let filterDiameter =
            (Array2D.length1 filter)
            / 2

        let filter = Helper.toFlatArray filter

        let clFilter =
            clContext.CreateClArray<float32>(
                filter,
                HostAccessMode.NotAccessible,
                DeviceAccessMode.ReadOnly
            )
...
let rotate (clContext: ClContext) (localWorkSize: int) =
    ...
    let rotateKernel = rotateKernel clContext localWorkSize
    let queue = clContext.QueueProvider.CreateQueue()
    ...
    fun (isClockwise: bool) (image: MyImage.MyImage) ->
        ...
        let result =
            queue.PostAndReply(fun ch ->
                Msg.CreateToHostMsg(
                    rotateKernel queue clWeight input image.Height image.Width output,
                    result,
                    ch
                )
            )
...
let flip (clContext: ClContext) (localWorkSize: int) =
    ...
    let flipKernel = flipKernel clContext localWorkSize
    let queue = clContext.QueueProvider.CreateQueue()
    ...
    fun (isVertical: bool) (image: MyImage.MyImage) ->
        ...
        let result =
            queue.PostAndReply(fun ch ->
                Msg.CreateToHostMsg(
                    flipKernel queue clWeight input image.Height image.Width output,
                    result,
                    ch
                )
            )
